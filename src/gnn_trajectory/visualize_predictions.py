...
def parse_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=1,
        help="Number of scenarios to visualize when --index is not set.",
    )
    p.add_argument(
        "--device",
        type=str,
...
def visualize_with_prediction(sample: Dict[str, Any], pred_traj: np.ndarray) -> None:
    ...
    pred_traj: (T, 2) numpy array with predicted future positions for the focal agent
    """
...
def main():
    ...
    def run_on_index(idx: int):
        ...
        print(f"[sample] idx={idx} scenario_id={sample['scenario_id']} "
              f"pred_traj shape={pred.shape} focal_idx={int(focal_idx[0])}")
